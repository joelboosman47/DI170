...
def display_message(): #defining the function
...
def favorite_book(title): 
    '''Takes one parameter and prints a statement using the parameter'''
    #print line using the parameter
    print(f"One of my favorite books is {title}.")

#call the function with an argument
favorite_book("The Chumash")
...
#defining the function with parameters (one of which has a default value)
def describe_city(city, country="Unknown"):
    #print message using the parameters
    print(f"{city} is in {country}.")

#calling with 2 arguments
describe_city('Amsterdam', 'the Netherlands')

#calling with only 1 argument
...
def take_number(your_number): #define the function
    random_int = randint(1, 100) #take random int between 1-100
    #compare input with random int
    if your_number == random_int: #if match, print this 
        print("Success!")
    else: print(f"Fail! Your number: {your_number}, Random number: {random_int}") #if no match, print different line and show the given argument and random int.
...
#Define a function to create a shirt with optional size and text parameters
def make_shirt(size='large', text='I love Python'):
    #Print the shirt details using the provided or default values
    print(f"The chose shirt has size: {size} with the message: {text}")

#Call the function with both size and text arguments (overrides defaults)
make_shirt(20, "some random text")

#Call the function with no arguments (uses both defaults)
make_shirt()

#Call the function with only size argument (text uses default)
make_shirt('medium')

#Call the function with both size and text arguments
make_shirt("small", 'I love Star Wars!')

#Call the function using keyword arguments (order doesn't matter)
make_shirt(text='Hello there!', size = "medium")
...
#a given list with names
magician_names = ['Harry Houdini', 'David Blaine', 'Criss Angel'] 

#define a function that takes a list and prints each value
def show_magicians(magician_names):
    for name in magician_names: #iterate through each value of input list
        print(name) #print values

#define function that takes list as parameter and changes the values
def make_great(magician_names):
    magician_names[:] = ["the Great " + name for name in magician_names] #add a string before each value of the parameter list
...
from random import uniform #import the necessary function

month = int(input("Please enter the month (number between 1-12): "))

#create a function that returns a random float (rounded to 1 decimal after the point) based on the given input
def get_random_temp(month):
    '''Returns a random float (rounded) between based on the given month'''
    if month > 12 or month < 3: #for the winter months  
        return round(uniform(-10.0, 15.9), 1)
    elif month > 2 and month < 6: #for spring months
        return round(uniform(16.0, 23.9), 1)
    elif month > 5 and month < 9: #for summer months
        return round(uniform(24.0, 40.0), 1)
    elif month > 8 and month < 12: #for authum months
        return round(uniform(16.0, 23.9), 1)

#creat function that calls the previous function anf prints a string
def main():
    temperature = get_random_temp(month)
    print(f'The temperature right now is {temperature} degrees Celcius.')

    #compare the variable to different float values and print a line depending on the value (range)
    if temperature < 0.0:
        print("Brrr, that’s freezing! Wear some extra layers today.")
    elif 0.0 <= temperature <= 15.9:
        print("Quite chilly! Don’t forget your coat.")
    elif 16.0 <= temperature <= 23.9:
        print("Nice weather.")
    elif 24.0 <= temperature <= 31.9:
