...
# Create a class called Farm. This class will represent a farm and its animals.
class Farm:
    def __init__(self, farm_name): #The Farm class should have an __init__ method. 
        self.name = farm_name #Inside __init__, create two attributes: name to store the farm’s name and animals to store the animals (initialize as an empty dictionary).
        self.animals = {}

    #Create a method called add_animal.
    def add_animal(self, animal_type, count=1): #It should take two parameters: animal_type and count (with a default value of 1).
        #If the animal_type already exists in the animals dictionary, increment its count by count.
        #else, add it to the dictionary as the key and with the given count as value.
        if animal_type not in self.animals:
            self.animals[animal_type] = count
        else: self.animals[animal_type] += count

    def get_info(self):
        farm_name = f'{self.name}\'s farm' #A string that displays the farm’s name
        
        #the animals and their counts (formatted)
        farm_animals = ''
        for key, value in self.animals.items():
            farm_animals += f'{key} : {value}\n'
        
        return f'{farm_name}\n\n{farm_animals}\n\tE-I-E-I-0!' #the “E-I-E-I-0!” phrase.
