...
keys = ['Ten', 'Twenty', 'Thirty'] #first list
values = [10, 20, 30] #second list
new_dict = dict(zip(keys, values)) #use zip to combine them and dict to store it as type dictionary
print(new_dict) #test the result
...
# Assuming the input string is like "rick:43,beth:13,morty:5,summer:8"
input_str = input("Enter family members as name:age, separated by commas: ")
family_dict = {}
total_cost = 0 

for pair in input_str.split(','):
    name, age = pair.split(':')
    family_dict[name.strip()] = int(age.strip())
    if 3 <= int(age.strip()) <= 12:
        total_cost += 10
...
type_of_clothes = ", ".join(brand['type_of_clothes'])
...
#3
dict3 = dict(zip(sorted(dict1.keys()), range(len(dict1))))
print(dict3)